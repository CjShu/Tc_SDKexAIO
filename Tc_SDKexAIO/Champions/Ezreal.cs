            ...
            if (Combo)
            {
                ...
                if (Menu["R"]["Auto"].GetValue<MenuBool>() && W.IsReady())
                {
                    ...
                    if (RTarget != null)
                    {
                        ...
                        else if (RTarget.IsValidTarget(Q.Range + E.Range - 200) && RTarget.Health + RTarget.MagicalShield + RTarget.HPRegenRate * 2 <= R.GetDamage(RTarget) + Q.GetDamage(RTarget) + W.GetDamage(RTarget) && Q.IsReady() && W.IsReady() && RTarget.CountAllyHeroesInRange(Q.Range + E.Range - 200) <= 1)
                        {
                            R.Cast(RTarget);
                        }
        ...
        private static void AutoRLogic(EventArgs args)
        {
            if (R.IsReady() && Menu["R"]["Auto"].GetValue<MenuBool>())
            {
                ...
                foreach (var t in GetEnemies(R.Range))
                {
                    ...
                    var boolean = t.Health + t.MagicalShield + t.HPRegenRate * 2 < RDMG;
