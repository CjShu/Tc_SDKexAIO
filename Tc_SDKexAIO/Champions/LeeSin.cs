namespace Tc_SDKexAIO.Champions
{
    ...
    internal static class LeeSin
    {
        ...
        internal static void Init()
        {
            ...
            var KeyMenu = Menu.Add(new Menu("Key", "按鍵設定"));
            {
                KeyMenu.GetKeyBind("Star", "非正規連招按鍵", Keys.T);
                KeyMenu.GetKeyBind("FleeW", "瞬眼按鍵 ", Keys.X);
                KeyMenu.GetKeyBind("RAuto", "自動R按鍵", Keys.L, KeyBindType.Toggle);
                KeyMenu.GetKeyBind("RFlash", "R閃現鼠標位置按鍵", Keys.Y);
                KeyMenu.GetKeyBind("Insec", "迴旋踢按鍵", Keys.R);
                KeyMenu.GetKeyBind("RAdv", "閃現R按鍵", Keys.A);
            }

            var QMenu = Menu.Add(new Menu("QMenu", "Q 按鍵設定"));
            {
                QMenu.GetSeparator("連招 設定");
                QMenu.GetBool("ComboQ", "使用Q1");
                QMenu.GetBool("ComboQ2", "使用Q2");
                QMenu.GetBool("ComboQ2Obj", "當Q2 快要消失時候使用", false);
                QMenu.GetBool("ComboQCol", "重撃 + Q");
                QMenu.GetBool("ComboStarKill", "當能擊殺敵人的時候自動非正規連招 Q", false);
                QMenu.GetSeparator("清線 清野");
                QMenu.GetBool("JungleQ", "使用 Q !");
                QMenu.GetBool("JungleQBig", "清野中只對大野怪使用");
                QMenu.GetSeparator("補刀 定設");
                QMenu.GetBool("LastHitQ", "使用Q !");
                QMenu.GetSeparator("擊殺 設定");
                QMenu.GetBool("KillStealQ", "使用Q !");
                QMenu.GetBool("KillStealQ2", "使用 Q2!");
                QMenu.GetSeparator("回旋踢 設置");
                QMenu.GetBool("InsecQ", "使用Q !");
                QMenu.GetBool("InsecQCol", "重撃 +Q");
                QMenu.GetBool("InsecQObj", " Q 目標附近的單位");
            }

            var WMenu = Menu.Add(new Menu("WMenu", "W 按鍵設定"));
            {
                WMenu.GetSeparator("連招 設定");
                WMenu.GetBool("ComboW", "使用 W !", false);
                WMenu.GetBool("ComboW2", "連招使用 W2!", false);
                WMenu.GetBool("StarKillWJ", "明星連招自動瞬眼", false);
                WMenu.GetBool("LaneJungleW", "清野使用W !", false);
            }

            var EMenu = Menu.Add(new Menu("EMenu", "E 按鍵設定"));
            {
                EMenu.GetSeparator("連招 設定");
                EMenu.GetBool("ComboE", "使用 E !");
                EMenu.GetBool("ComboE2", "使用E2 緩速");
                EMenu.GetSeparator("清線 清野");
                EMenu.GetBool("LaneJungleE", "使用E 清野 !");
                EMenu.GetSeparator("擊殺 設定");
                EMenu.GetBool("KSE", "使用 !");
            }

            var RMenu = Menu.Add(new Menu("RMenu", "R 按鍵設定"));
            {
                RMenu.GetBool("KSR", " 自動R 擊殺");
                RMenu.GetBool("RAutoKill", "最好能R彈到能擊殺的敵人");
                RMenu.GetSlider("RAutoCountA", "最小命中敵人數 >=", 1, 1, 4);
                RMenu.Add(new MenuSeparator("RLSITST", "R英雄列表"));
            ...
            var Insec = Menu.Add(new Menu("Insec", "非正規回旋踢"));
            {
                Insec.GetBool("TargetSelect", "僅回旋踢已選擇的目標", false);
                Insec.GetList("Mode", "回旋踢模式", new[] { "炮塔/對友 位置", "鼠標位置" });
                Insec.GetBool("Flash", "使用閃現");
                Insec.GetList("FlashMode", "閃現模式", new[] { "R > 閃", "閃 > R", "兩者" });
                Insec.GetBool("FlashJump", "使用瞬眼來突進到 R 閃目標");
            }

            var drawMenu = Menu.Add(new Menu("Draw", "顯示設定"));
            {
                drawMenu.GetBool("Q", "Q 範圍", false);
                drawMenu.GetBool("W", "W 範圍", false);
                drawMenu.GetBool("E", "E 範圍", false);
                drawMenu.GetBool("R", "R 範圍", false);
                drawMenu.GetBool("KnockUp", "自動R 擊飛狀態");
                drawMenu.GetBool("DLine", "回旋踢直線");
                drawMenu.GetBool("DWardFlash", " 回旋踢範圍");
            }
        ...
        internal static bool CanR(Obj_AI_Hero target)
        {
        ...
        internal static void CastQSmite(Obj_AI_Hero target)
        {
        ...
        internal static void CastRFlash(Obj_AI_Base target)
        {
            ...
            if (!targetSelect.IsValidTarget() || !targetSelect.Compare(target) || target.Health + target.PhysicalShield <= R.GetDamage(target))
            {
                return;
            }
            var pos = new Vector3();
            ...
            {
                pos = GetPositionKickTo((Obj_AI_Hero)target);
            }
        ...
        internal static void Combo()
        {
            if (R.IsReady() && Menu["QMenu"]["ComboStarKill"] && Q.IsReady() && !IsQOne && Menu["QMenu"]["ComboQ"] && Menu["QMenu"]["ComboQ2"])
            {
                ...
                if (target != null && target.Health + target.PhysicalShield > Q.GetDamage(target, DamageStage.SecondCast) + Player.GetAutoAttackDamage(target) && target.Health + target.PhysicalShield <= GetQ2Dmg(target, R.GetDamage(target)) + Player.GetAutoAttackDamage(target))
                {
        ...
        internal static float GetRColDmg(Obj_AI_Hero kickTarget, Obj_AI_Hero hitTarget)
        {
            return R.GetDamage(hitTarget) + (float)Player.CalculateDamage(hitTarget, DamageType.Physical, new[] { 0.12, 0.15, 0.18 }[R.Level - 1] * kickTarget.BonusHealth);
        }
        ...
        internal static void KillSteal()
        {
            if (Menu["QMenu"]["KillStealQ"] && Q.IsReady())
            {
                if (IsQOne)
                {
                    ...
                    if (target != null && (target.Health + target.PhysicalShield <= Q.GetDamage(target) || (Menu["QMenu"]["KillStealQ2"] && target.Health + target.PhysicalShield <= GetQ2Dmg(target, Q.GetDamage(target)) + Player.GetAutoAttackDamage(target) && Player.Mana - Q.Instance.ManaCost >= 30)) && Q.Casting(target, false, CollisionableObjects.Heroes | CollisionableObjects.Minions | CollisionableObjects.YasuoWall).IsCasted())
                    {
                        return;
                    }
                }
                else if (Menu["QMenu"]["KillStealQ2"] && !IsDashing)
                {
                    ...
                    if (target != null && target.Health + target.PhysicalShield <= Q.GetDamage(target, DamageStage.SecondCast) + Player.GetAutoAttackDamage(target) && Q2.Cast())
                    {
                        isDashing = true;
                        return;
                    }
            ...
            if (Menu["EMenu"]["KSE"] && E.IsReady() && IsEOne && Variables.TargetSelector.GetTargets(E.Range, E.DamageType).Any(i => E.CanHitCircle(i) && i.Health + i.MagicalShield <= E.GetDamage(i)) && E.Cast())
            {
                return;
            }
            ...
            if (Menu["RMenu"]["KSR"] && R.IsReady())
            {
                ...
                if (targetList.Count > 0)
                {
                    ...
                    {
                        var targetQ2R = targetList.FirstOrDefault(i => HaveQ(i) && i.Health + i.PhysicalShield <= GetQ2Dmg(i, R.GetDamage(i)) + Player.GetAutoAttackDamage(i));
        ...
        internal static void OnDraw(EventArgs args)
        {
            ...
            if (Menu["Draw"]["KnockUp"] && R.Level > 0)
            {
                ...
                Drawing.DrawText(
                    pos.X - (float)Drawing.GetTextExtent(text)
                    .Width / 2,
            ...
            switch (Variables.Orbwalker.ActiveMode)
            {
                ...
                case OrbwalkingMode.None:
                    if (Menu["Key"]["RFlash"].GetValue<MenuKeyBind>().Active)
                    {
                        ...
                        {
                            var target = Variables.TargetSelector.GetTargets(R.Range, R.DamageType).Where(i => i.Health + i.PhysicalShield > R.GetDamage(i)).MaxOrDefault(i => new Priority().GetPriority(i));
        ...
        internal static void StarCombo()
        {
            ...
            if (Q.IsReady())
            {
                ...
                else if (!IsDashing && HaveQ(target) && (target.Health + target.PhysicalShield <= Q.GetDamage(target, DamageStage.SecondCast) + Player.GetAutoAttackDamage(target) || (!R.IsReady() && IsRecentR && CanR(target))) && Q2.Cast())
                {
                    isDashing = true;
                    return;
                }
        ...
        /*
        internal static void UseItem(Obj_AI_Hero target)
        {
        ...
        */
        ...
        internal static void GapByFlash(Obj_AI_Hero target, Vector3 posBehind)
        {
        ...
        internal static void GapByFlashR(Obj_AI_Hero target, Vector3 posBehind)
        {
        ...
        internal static void GapByQ(Obj_AI_Hero target)
        {
        ...
        internal static void GapByRFlash(Obj_AI_Hero target)
        {
            if (!R.CastOnUnit(target))
            {
                return;
        ...
        internal static void GapByWardJump(Obj_AI_Hero target, Vector3 posBehind)
        {
        ...
        internal static Tuple<Vector3, bool> GapCheck(Obj_AI_Hero target, bool useFlash = false)
        {
        ...
        internal static Vector3 GetPositionAfterKick(Obj_AI_Hero target)
        {
        ...
        internal static Vector3 GetPositionBehind(Obj_AI_Hero target)
        {
